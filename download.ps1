<#
    ...
    Alt:     irm https://raw.githubusercontent.com/AB-invisible/opensteam-app/main/download.ps1 | iex
...
[CmdletBinding()]
param(
    [string] $Repo          = 'AB-invisible/opensteam-app',
    [string] $AssetName     = 'OpenSteamApp.exe',
...
# When invoked via `irm ... | iex`, install immediately instead of showing the menu.
if (-not $Action -and -not $MyInvocation.MyCommand.Path) {
    $Action = 'install'
}
...
function Install-ReleaseAssets {
    ...
    foreach ($name in @('AppIcon.ico', 'OpenSteamAppLogo.png', 'OpenSteamMark.png')) {
...
function Get-ShortcutIconLocation {
    ...
    return $script:ExePath
}

function Get-LatestReleaseFromJson {
    param(
        [string] $SourceLabel,
        [string] $Url
    )

    Write-Step "Fetching latest release from $SourceLabel"
    $doc = Invoke-RestMethod $Url -Headers @{ 'User-Agent' = 'OpenSteam-Console' }
    if (-not $doc.version) { return $null }

    $tag = if ($doc.tag) { $doc.tag } else { "v$($doc.version.TrimStart('v','V'))" }
    $downloadUrl = if ($doc.downloadUrl) {
        $doc.downloadUrl
    } else {
    ...
    return [PSCustomObject]@{
        tag_name = $tag
        html_url = if ($doc.releaseUrl) { $doc.releaseUrl } else { "https://github.com/$Repo/releases/tag/$tag" }
        assets   = @(
            [PSCustomObject]@{
                name                 = $AssetName
...
function Get-LatestRelease {
    ...
    try {
        ...
        } elseif (-not $script:Latest.assets) {
            Write-Warn "GitHub API response does not contain 'assets'."
        } else {
    ...
    foreach ($fallback in @(
            @{ Label = 'latest.json';    Url = "https://raw.githubusercontent.com/$Repo/main/latest.json" },
            @{ Label = 'OpenSteam API'; Url = 'https://manifest-web-ylio.onrender.com/api/v2/app/latest' }
        )) {
        try {
            $script:Latest = Get-LatestReleaseFromJson -SourceLabel $fallback.Label -Url $fallback.Url
            if ($script:Latest) { return $script:Latest }
        } catch {
            Write-Warn "$($fallback.Label) failed: $($_.Exception.Message)"
        }
    }

    Write-Fail "Failed to fetch latest release from all sources."
    return $null
}
...
function Write-Banner {
    ...
    Write-Color "O P E N S T E A M   A P P" Magenta -NoNewline
    Write-Color "   ·   " DarkGray  -NoNewline
    Write-Color "O P E N S T E A M" Blue   -NoNewline
    Write-Color "          │" Magenta
    Write-Color "   │                                                                  │" Magenta
    Write-Color "   └──────────────────────────────────────────────────────────────────┘" Magenta
    Write-Host ""
...
function Invoke-Install {
    ...
    try {
        ...
        Install-ReleaseAssets -Release $rel
