...
public sealed partial class SettingsPage : Page
{
    ...
    {
        InitializeComponent();
        PairingPanel.PairingCompleted += PairingPanel_PairingCompleted;
        Loaded += SettingsPage_Loaded;
    }

    private async void PairingPanel_PairingCompleted(object? sender, EventArgs e)
    {
    ...
    private async void CheckUpdates_Click(object sender, RoutedEventArgs e)
    {
        ...
        try
        {
            var outcome = await TypedApp.Svcs.UpdateChecker.CheckDetailedAsync();

            if (outcome.Result is null)
            {
                UpdateInfoBar.Severity = InfoBarSeverity.Warning;
                UpdateInfoBar.Title   = "Could not check for updates";
                UpdateInfoBar.Message = string.IsNullOrWhiteSpace(outcome.ErrorMessage)
                    ? "Make sure you're connected to the internet."
                    : $"{outcome.ErrorMessage} You can also run download.ps1 from the opensteam-app repo.";
            }
            else if (outcome.Result.IsUpdateAvailable)
            {
                UpdateInfoBar.Severity = InfoBarSeverity.Informational;
            ...
            else
            {
                ...
                UpdateInfoBar.Message = $"v{outcome.Result.CurrentVersion.ToString(3)} is the latest release.";
            }
